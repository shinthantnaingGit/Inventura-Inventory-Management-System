  ...
  const info = [
    {
      ...
      value: t("landing.contact.info.location.value", "インベンチュラ神田ビル、東京都千代田区神田小川町2-4-8、〒101-0052"),
      href: t("landing.contact.info.location.href", "https://maps.app.goo.gl/YourLocationLink"),
    ...
    },
    {
      ...
      value: t("landing.contact.info.hours.value", "月曜 - 金曜: 午前9時 - 午後6時 (JST)"),
      href: "#",
    ...
    },
    {
      ...
      href: t("landing.contact.info.social.href", "https://twitter.com/inventura_jp"),
      icon: <LinkIcon className="size-5 text-blue-500" />,
    },
  ];

  return (
    <section ref={contactRef} id="contact" className="bg-white dark:bg-gray-800 text-gray-900 dark:text-white py-20 lg:py-32 transition-colors duration-500">
      <div className="container mx-auto px-4">
        ...
        >
          <h2 className="text-3xl lg:text-4xl font-bold mb-4">{t("landing.contact.heading", "お問い合わせ")}</h2>
          <p className="text-lg text-gray-600 dark:text-gray-300">{t("landing.contact.subheading", "サポートやご質問がある場合は、お気軽にお問い合わせください。")}</p>
        </motion.div>
        ...
        >
          {info.map((item, index) => (
            <div key={index} className="flex items-start space-x-3 text-lg text-gray-700 dark:text-gray-300">
              <div className="p-2 rounded-full bg-blue-100 dark:bg-blue-900 text-blue-600 dark:text-blue-300">{item.icon}</div>
              <div>
                <h3 className="font-semibold">{item.label}</h3>
                <a href={item.href} className="text-sm hover:text-blue-600 dark:hover:text-blue-400 transition-colors duration-300">
                  {item.value}
                </a>
              </div>
            </div>
